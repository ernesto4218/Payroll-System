if (document.getElementById("search-table") && typeof simpleDatatables.DataTable !== 'undefined') {
    ...
    search.innerHTML += `<button id="addloadbtn" type="button" class="flex flex-row items-center gap-2 text-white bg-emerald-700 hover:bg-emerald-800 focus:ring-4 focus:ring-emerald-300 font-medium rounded-lg text-xs px-3 py-2">
        <svg class="w-3 h-3" fill='currentColor' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path d="M256 80c0-17.7-14.3-32-32-32s-32 14.3-32 32l0 144L48 224c-17.7 0-32 14.3-32 32s14.3 32 32 32l144 0 0 144c0 17.7 14.3 32 32 32s32-14.3 32-32l0-144 144 0c17.7 0 32-14.3 32-32s-14.3-32-32-32l-144 0 0-144z"/></svg>
    Add</button>
    <button id="addsubjectbtn" type="button" class="flex flex-row items-center gap-2 text-white bg-emerald-600 hover:bg-emerald-800 focus:ring-4 focus:ring-emerald-300 font-medium rounded-lg text-xs px-3 py-2">
        <svg class="w-3 h-3" fill='currentColor' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path d="M256 80c0-17.7-14.3-32-32-32s-32 14.3-32 32l0 144L48 224c-17.7 0-32 14.3-32 32s14.3 32 32 32l144 0 0 144c0 17.7 14.3 32 32 32s32-14.3 32-32l0-144 144 0c17.7 0 32-14.3 32-32s-14.3-32-32-32l-144 0 0-144z"/></svg>
    Subject</button>
    `;

    const addsubjectformparent = document.getElementById('addsubjectformparent');
    const addsubjectbtn = document.getElementById('addsubjectbtn');
    const closeaddsubjectform = document.getElementById('closeaddsubjectform');
    const addsubjectform = document.getElementById('addsubjectform');
    const addloadbtn = document.getElementById('addloadbtn');
    const addloadformparent = document.getElementById('addloadformparent');
    const closeaddloadform = document.getElementById('closeaddloadform');
    const addloadform = document.getElementById('addloadform');

    addsubjectbtn.onclick = function() {
        addsubjectformparent.classList.add('flex');
        addsubjectformparent.classList.remove('hidden');
    };

    closeaddsubjectform.onclick = function() {
        addsubjectformparent.classList.add('hidden');
        addsubjectformparent.classList.remove('flex');
    };
    ...
    addloadform.addEventListener('submit', function (e) {
        e.preventDefault(); // prevent page reload

        const formData = new FormData(addloadform);
        const jsonData = Object.fromEntries(formData.entries());

        fetch('/api/add-faculty-load', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
    ...
    addloadbtn.onclick = function() {
        addloadformparent.classList.add('flex');
        addloadformparent.classList.remove('hidden');
    };

    closeaddloadform.onclick = function() {
        addloadformparent.classList.add('hidden');
        addloadformparent.classList.remove('flex'); 
    };


    document.querySelectorAll('.deleteloadbtn').forEach(btn => {
        btn.onclick = function() {
            const id = btn.getAttribute('data-id');
            fetch('/api/delete-faculty-load', {
                method: 'POST',
...
if (document.getElementById("subject-table") && typeof simpleDatatables.DataTable !== 'undefined') {
